...
from KatilMusic.utils import first_page, second_page
...
from config import KATIL_USERS, START_IMG_URL, SUPPORT_CHAT
...
@Client.on_message(filters.command(["help"]) & filters.private & ~KATIL_USERS)
@Client.on_callback_query(filters.regex("settings_back_helper") & ~KATIL_USERS)
...
@Client.on_message(filters.command(["help"]) & filters.group & ~KATIL_USERS)
@LanguageStart
...
@Client.on_callback_query(filters.regex("help_callback") & ~KATIL_USERS)
@languageCB
    ...
    if cb == "hb9":
        if CallbackQuery.from_user.id not in SUDOERS:
            return await CallbackQuery.answer(
                "😎𝗣𝗔𝗛𝗟𝗘 𝗜𝗧𝗭_𝗞𝗔𝗔𝗧𝗜𝗟 𝗞𝗢 𝗣𝗔𝗣𝗔 𝗕𝗢𝗟 𝗝𝗔𝗞𝗘 😆😆", show_alert=True
            )
        else:
...
@Client.on_callback_query(filters.regex("dilXaditi") & ~KATIL_USERS)
@languageCB
