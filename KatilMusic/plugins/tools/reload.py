...
from config import KATIL_USERS, adminlist, lyrical

rel = {}


@app.on_message(
    filters.command(["admincache", "reload", "refresh"]) & filters.group & ~KATIL_USERS
)
@language
...
@app.on_message(filters.command(["reboot"]) & filters.group & ~KATIL_USERS)
@AdminActual
...
@app.on_callback_query(filters.regex("close") & ~KATIL_USERS)
...
@app.on_callback_query(filters.regex("stop_downloading") & ~KATIL_USERS)
@ActualAdminCB
